...
// ";
    ...
    let results : Vec<usize> = process(contents.to_string());

    let mut i = 1;
    for result in results {
        println!("Seed {}: {}",i , result);
        i += 1;
    }
}

fn process(contents: String) -> Vec<usize> {
    ...
    for seed in seeds {
        let cloned_conversion_map = conversion_map.clone();
        handles.push({
            std::thread::spawn(move || {
                return find_min(seed, cloned_conversion_map.clone());
            })
        })
    ...
    for handle in handles {
        min_arr.push(handle.join().ok().unwrap())
    }

    min_arr
}

fn find_min(seed: Seed, conversion_map: ConversionMap) -> usize {
    let mut min: usize = usize::MAX;

    for i in seed.range_start..(seed.range_start + seed.length) {
        min = min.min(conversion_map.convert(i));
    }
